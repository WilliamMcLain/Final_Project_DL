...
def plot_pr_with_optimal_points(model, testloader, device):
    """
    Plots PR curve with F1/F2 contours and highlights the optimal F1 and F2 points.
    Also prints the optimal thresholds to the console.
    """
    model.eval()
    model.to(device)
    ...
    with torch.no_grad():
        for inputs, labels in testloader:
            ...
            # Convert to probabilities (Change to sigmoid if using BCEWithLogits)
    ...
    # --- 3. Find Best F1 and F2 Points ---

    # Handle division by zero for the very first item (usually 0 precision/recall)
    numerator = 2 * recall * precision
    denominator = recall + precision
    f1_scores = np.divide(numerator, denominator, out=np.zeros_like(denominator), where=denominator != 0)

    beta = 2
    numerator_f2 = (1 + beta ** 2) * recall * precision
    denominator_f2 = (beta ** 2 * precision) + recall
    f2_scores = np.divide(numerator_f2, denominator_f2, out=np.zeros_like(denominator_f2), where=denominator_f2 != 0)

    # Get the index of the best scores
    ...
    # Extract Thresholds (Note: thresholds array is 1 shorter than P/R arrays)
    # We use min(index, len-1) to be safe, though argmax rarely picks the last dummy point.
    thresh_f1 = thresholds[min(ix_f1, len(thresholds) - 1)]
    thresh_f2 = thresholds[min(ix_f2, len(thresholds) - 1)]

    print(f"Best F1: {f1_scores[ix_f1]:.4f} at Threshold: {thresh_f1:.4f}")
    print(f"Best F2: {f2_scores[ix_f2]:.4f} at Threshold: {thresh_f2:.4f}")
    ...
    # Calculate Background F-Surfaces
    F1_grid = 2 * (P * R) / (P + R)
    F2_grid = (1 + beta ** 2) * (P * R) / ((beta ** 2 * P) + R)

    # F1 Contours
    CS_f1 = plt.contour(R, P, F1_grid, levels=np.linspace(0.1, 0.9, 9), colors='green', linestyles='--', alpha=0.3)
    plt.clabel(CS_f1, inline=True, fontsize=9, fmt='F1=%.1f')

    # F2 Contours
    CS_f2 = plt.contour(R, P, F2_grid, levels=np.linspace(0.1, 0.9, 9), colors='blue', linestyles=':', alpha=0.3)
    plt.clabel(CS_f2, inline=True, fontsize=9, fmt='F2=%.1f')

    # Plot Curve and Optimal Points
    plt.plot(recall, precision, color='black', lw=2, label='PR Curve')

    # Plot Best F1
    plt.scatter(best_f1_pt[0], best_f1_pt[1], s=200, marker='*', color='green', label=f'Best F1 ({f1_scores[ix_f1]:.2f})', zorder=5)

    # Plot Best F2
    plt.scatter(best_f2_pt[0], best_f2_pt[1], s=100, marker='o', color='blue',
                label=f'Best F2 ({f2_scores[ix_f2]:.2f})', zorder=5)

    # Formatting
    plt.title('Precision-Recall Curve with Optimal F1/F2 Points', fontsize=16)
...
if __name__ == "__main__":
    ...
    plot_pr_with_optimal_points(model, loaders['test'], 'cpu')
